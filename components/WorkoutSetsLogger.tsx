...
const WorkoutSetsLogger: React.FC<WorkoutSetsLoggerProps> = ({
    ...
    return (
        <div className="relative flex h-full w-full flex-col bg-background-light dark:bg-background-dark overflow-y-auto pb-20 font-display transition-colors duration-300">
            {/* Exercise Header */}
            <div className="px-4 pt-2 pb-3 shrink-0">
                <h2 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white">{exerciseName}</h2>
            ...
            <div className="flex flex-col gap-2 mt-4 px-3 sm:px-4">
                {/* Labels */}
                <div className="flex items-center px-1 sm:px-2 py-1 text-slate-500 dark:text-[#90adcb] text-[9px] sm:text-[10px] font-bold uppercase tracking-wider">
                ...
                {sets.map((set, index) => (
                    <div
                        ...
                        className={`flex items-center gap-1 sm:gap-2 p-2 sm:p-3 rounded-lg sm:rounded-xl border border-transparent transition-all ${set.completed
                                ? 'bg-slate-100 dark:bg-[#182634] opacity-80'
                                : 'bg-white dark:bg-[#182634] border-2 border-[var(--color-primary)]/30 shadow-sm'
                            }`}
                    >
                        <div className={`w-8 sm:w-10 font-bold text-xs sm:text-sm ${set.completed ? 'text-slate-500 dark:text-[#90adcb]' : 'text-[var(--color-primary)]'}`}>
                            {index + 1}
                        </div>
                        <div className="flex-1 px-1 sm:px-2 text-[10px] sm:text-xs text-slate-400 dark:text-slate-500 hidden sm:block">
                            {set.previous || '-'}
                        </div>
                        <div className="w-18 sm:w-20">
                            <input
                                className={`w-full rounded-lg h-11 sm:h-12 text-center text-sm font-bold outline-none border focus:border-[var(--color-primary)] focus:ring-1 focus:ring-[var(--color-primary)] transition-colors ${set.completed
                                        ? 'bg-slate-200 dark:bg-[#223649]/50 text-slate-500 border-none'
                                        : 'bg-slate-50 dark:bg-[#223649] text-slate-900 dark:text-white border-slate-200 dark:border-slate-700'
                                    }`}
                                value={set.weight}
                        ...
                        <div className="w-11 sm:w-12 flex justify-end">
                            <button
                                ...
                                className={`size-9 sm:size-10 flex items-center justify-center rounded-lg border-2 transition-colors ${set.completed
                                        ...
                                        : 'bg-transparent border-slate-300 dark:border-[#314d68] text-transparent hover:border-[var(--color-primary)]'
                                    }`}
                            >
                ...
                <button
                    onClick={onAddSet}
                    className="mt-2 flex items-center justify-center gap-1 sm:gap-2 py-2.5 sm:py-3 rounded-lg sm:rounded-xl border-2 border-dashed border-slate-300 dark:border-[#314d68] text-slate-500 dark:text-[#90adcb] hover:bg-slate-200 dark:hover:bg-[#223649] transition-colors font-bold text-xs sm:text-sm"
                >
